...
def mark_invoice(
    book_token: str, invoice_id: str, organization_id: str, status : Literal['sent','draft','void']
) -> requests.Response:
    ...
    response = requests.post(f"https://www.zohoapis.ca/books/v3/invoices/{invoice_id}/status/{status}?organization_id={organization_id}", 
    headers=get_book_headers(book_token=book_token))
...
def fetch_invoice(
    invoice_id: str, book_token: str, organization_id: str, formate : Literal['pdf','json','html']
) -> requests.Response:
    ...
    """
    response = requests.get(
        f"https://www.zohoapis.ca/books/v3/invoices/{formate}?organization_id={organization_id}&invoice_ids={invoice_id}",
        headers=get_book_headers(book_token=book_token),
    )
